    ...
    <div className="max-w-4xl mx-auto pb-20">
      <div className="text-center mb-8 sm:mb-12">
        <motion.div 
          ...
          className="w-20 h-20 sm:w-24 sm:h-24 bg-indigo-600 rounded-2xl sm:rounded-3xl flex items-center justify-center text-white mx-auto mb-4 sm:mb-6 shadow-2xl shadow-indigo-200 dark:shadow-none"
        >
          <Sparkles size={40} />
        </motion.div>
        <h2 className="text-3xl sm:text-4xl font-black mb-2 text-gray-900 dark:text-white">Quiz abgeschlossen!</h2>
        <p className="text-gray-500 dark:text-gray-400 text-base sm:text-lg">Gute Arbeit bei "{pkg.name}"</p>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-8 sm:mb-12">
        <div className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-gray-100 dark:border-gray-700 shadow-sm text-center transition-colors">
          <p className="text-[10px] sm:text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 sm:mb-2">Score</p>
          <p className="text-4xl sm:text-5xl font-black text-indigo-600 dark:text-indigo-400">{result.score}<span className="text-xl sm:text-2xl text-gray-300 dark:text-gray-600">/{result.total}</span></p>
        </div>
        <div className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-gray-100 dark:border-gray-700 shadow-sm text-center transition-colors">
          <p className="text-[10px] sm:text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 sm:mb-2">Genauigkeit</p>
          <p className="text-4xl sm:text-5xl font-black text-emerald-500 dark:text-emerald-400">{Math.round(result.accuracy)}%</p>
        </div>
        <div className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-gray-100 dark:border-gray-700 shadow-sm text-center flex flex-col justify-center transition-colors">
          <p className="text-[10px] sm:text-sm font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest mb-1 sm:mb-2">Status</p>
          <p className="text-xl sm:text-2xl font-black text-gray-800 dark:text-white">
            {result.accuracy >= 80 ? "Exzellent!" : result.accuracy >= 50 ? "Gut gemacht" : "Weiter üben"}
          </p>
          <button 
            onClick={onShowStudyGuide}
            className="mt-3 min-h-[44px] text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300 flex items-center justify-center gap-2 group"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-12">
        {/* Strengths */}
        <div className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-sm transition-colors">
          <div className="flex items-center gap-3 mb-4 sm:mb-6">
            <div className="w-9 h-9 sm:w-10 sm:h-10 bg-emerald-50 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400 rounded-xl flex items-center justify-center">
        ...
        <div className="bg-white dark:bg-gray-800 p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border border-gray-100 dark:border-gray-700 shadow-sm transition-colors">
          <div className="flex items-center gap-3 mb-4 sm:mb-6">
            <div className="w-9 h-9 sm:w-10 sm:h-10 bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 rounded-xl flex items-center justify-center">
      ...
      {/* Next Steps */}
      <div className="bg-indigo-600 p-8 sm:p-10 rounded-[2.5rem] sm:rounded-[3rem] text-white shadow-2xl shadow-indigo-200 dark:shadow-none">
